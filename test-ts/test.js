...
const hjs_client_1 = __importDefault(require("hjs-client"));
// 测试：创建客户端实例（应该有类型提示）
const client = new hjs_client_1.default('https://hjs-api.onrender.com');
async function runTest() {
    try {
        // 测试1：recordJudgment 应该有完整的参数提示
        console.log('🧪 测试 recordJudgment...');
        const record = await client.recordJudgment('test@example.com', 'test_action', { test: true, source: 'typescript-test' });
